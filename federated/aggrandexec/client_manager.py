...
from federated.aggrandexec.internal.client import Client
...
class clientManager(object):
    ...
    def __init__(self, mode, args, sample_seed=233):
        ...
        if self.mode == 'oort':
            from thirdparty.oort.training_select import create_training_selector
    ...
    def register_client(self, hostId: int, clientId: int, size: int, speed: Dict[str, float], duration: float=1) -> None:
        ...
        if size >= self.filter_less and size <= self.filter_more:
            ...
            if self.mode == "oort":
                ...
                self.ucbSampler.update_client(clientId, feedbacks=feedbacks)
        else:
    ...
    def select_participants(self, num_of_clients: int, cur_time: float=0) -> List[int]:
        ...
        if self.mode == "oort" and self.count > 1:
            pickled_clients = self.ucbSampler.select_client(
                num_of_clients, avail_clients=clients_online_set)
        else:
    ...
    def getAllMetrics(self):
        if self.mode == "oort":
            return self.ucbSampler.get_clients()
        return {}
    ...
    def getClient(self, clientId):
        clients = self.ucbSampler.get_clients()
        return clients[clientId]

    def get_mean_reward(self):
        if self.mode == 'oort':
            return self.ucbSampler.mean_reward()
        return 0.
