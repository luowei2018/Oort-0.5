...
from federated.aggrandexec.aggregation.optimizers import ServerOptimizer
from federated.aggrandexec.client_manager import clientManager
# libs from federated
import federated.aggrandexec.config_parser as parser
from federated.dataloaders.divide_data import DataPartitioner, select_dataset
from federated.dataloaders.utils_data import get_data_transform
from federated.utils.model_test_module import test_model
# federated model libs
from federated.utils.models.model_provider import get_cv_model
...
def import_libs():
    ...
    if parser.args.task == 'nlp' or parser.args.task == 'text_clf':
        ...
        from federated.dataloaders.nlp import load_and_cache_examples, mask_tokens
        ...
        from federated.dataloaders.speech import SPEECH, BackgroundNoiseDataset
        from federated.dataloaders.transforms_stft import (AddBackgroundNoiseOnSTFT,
                                                        DeleteSTFT,
                                                        FixSTFTDimension,
                                                        StretchAudioOnSTFT,
                                                        ...
                                                        ToSTFT)
        from federated.dataloaders.transforms_wav import (ChangeAmplitude,
        ...
        from federated.dataloaders.rcnn.lib.datasets.factory import get_imdb
        from federated.dataloaders.rcnn.lib.datasets.pascal_voc import readClass
        from federated.dataloaders.rcnn.lib.model.faster_rcnn.resnet import resnet
        from federated.dataloaders.rcnn.lib.model.roi_layers import nms
        from federated.dataloaders.rcnn.lib.model.rpn.bbox_transform import (
            bbox_transform_inv, clip_boxes)
        from federated.dataloaders.rcnn.lib.model.utils.config import (
            cfg, cfg_from_file, cfg_from_list, get_output_dir)
        from federated.dataloaders.rcnn.lib.model.utils.net_utils import (
            adjust_learning_rate, clip_gradient, load_net, save_checkpoint,
            save_net, weights_normal_init)
        from federated.dataloaders.rcnn.lib.roi_data_layer.roibatchLoader import \
            roibatchLoader
        from federated.dataloaders.rcnn.lib.roi_data_layer.roidb import \
            combined_roidb
        ...
        from federated.dataloaders.dqn import RLData, Net, DQN
...
#          model = AutoModelWithLMHead.from_config(config)
        ...
        if parser.args.model == 'albert':
            ...
            from federated.utils.models.simple.models import LogisticRegression
        ...
        if parser.args.model == 'mobilenet':
            from federated.utils.models.specialized.resnet_speech import \
                mobilenet_v2
        ...
        from federated.utils.models.specialized.voice_model import (
        ...
        if parser.args.model == "lr":
            ...
            from federated.utils.models.simple.models import LinearSVM
            model = LinearSVM(parser.args.input_dim, outputClass[parser.args.data_set])
        else:
            if parser.args.model_zoo == "federated-zoo":
...
def init_dataset():
    ...
    if parser.args.task == "detection":
        ...
        if parser.args.data_set == 'Mnist':
            ...
            from federated.dataloaders.femnist import FEMNIST
            ...
            from federated.dataloaders.openimage import OpenImage
            ...
            from federated.dataloaders.stackoverflow import stackoverflow
            ...
            if parser.args.model == 'albert':
                import federated.dataloaders.amazon as fl_loader
                ...
                import federated.dataloaders.word2vec as fl_loader
            ...
            import federated.dataloaders.yelp as fl_loader
            ...
            from federated.dataloaders.voice_data_loader import \
                SpectrogramDataset
